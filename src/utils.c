...
ssize_t prompt_hidden_tty(char* buf, size_t cap, const char* prompt) {
    ...
    if (tcgetattr(tty, &oldt)) {
        close(tty);
        return -1;
    }
    newt = oldt;
    newt.c_lflag &= ~ECHO;
    if (tcsetattr(tty, TCSAFLUSH, &newt)) {
        close(tty);
        return -1;
    }
...
unsigned int seed_prng_from_build(void) {
    ...
    unsigned int s =
        (unsigned int)(time(NULL) ^ getpid() ^ (unsigned int)(BUILD_SEED & 0xffffffff));
    return s;
}
...
static void seed_prng_once(void) {
    ...
    if (xorshift64star_state == 0)
        xorshift64star_state = (uint64_t)SAFE_DEBUG_PRNG_SEED;
    return;
#endif
    if (xorshift64star_state != 0)
        return;
